...
def generate_pdf(data, hostname, start_date, end_date, pdf_filename):
    ...
    # Add a title
    title = Paragraph("Device Downtime Report", styles['Title'])
    ...
    log_entries = data["Host Log Entries"]
    table_data = [['Downtime Start', 'Downtime End', 'Total Time']]  # Table header
    for entry in log_entries:
        table_data.append([entry['Event Start Time'], entry['Event End Time'], entry['Event Type']])
...
def main():
    ...
    try:
        ...
        generate_pdf(json_output, hostname, fromdate, todate, pdf_filename)
