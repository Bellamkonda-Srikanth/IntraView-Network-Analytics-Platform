...
def get_downtime_information(conn, systemname, start_date, end_date):
    ...
    cursor = conn.cursor()
    query = f"""SELECT hostaddress, rca, reportingdate, human_readable FROM bb_outage_tracking AS ot WHERE ot.hostaddress = ( SELECT dd.publicip FROM bb_device_details AS dd WHERE dd.systemname = '{systemname}' ) AND ot.reportingdate BETWEEN '{start_date}' AND '{end_date}';"""
...
def generate_pdf(data, hostname, start_date, end_date, pdf_filename, dt_info):
    ...
    table_data = [['Hostname', 'Hostaddres', 'Reporting Time', 'Downtime', 'Root Cause']]  # Table header
    for entry in dt_info:
        table_data.append([hostname, entry[0], entry[2], entry[3], entry[1]])   
    ...
    for row_index in range(1, len(table_data)):
        table.setStyle(TableStyle([
            ('BACKGROUND', (4, row_index), (4, row_index), colors.green),
            ('TEXTCOLOR', (4, row_index), (4, row_index), colors.whitesmoke),
            ('BACKGROUND', (3, row_index), (3, row_index), colors.red),
